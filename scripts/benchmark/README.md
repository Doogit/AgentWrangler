...
Beyond controller-observed HTTP timing it reports, from the child's vantage: per-request route service time and SQLite statement-execution counts (`child_phases`), per-scale `EXPLAIN QUERY PLAN` output for every distinct executed statement (`query_plans`), and a real populated ingest catch-up over a synthetic JSONL corpus with concurrent HTTP interference reads (`ingest_catchup`). `browser-measure.ts` measures cold/warm navigations for four routes at all three scales (requires `npm run build:ui`), application query-cache retention, live moving-preset cache behavior, and React commit durations from the profiling bundle (requires `npm run build:ui:profiling`). Current acceptance-budget state and the freeze protocol live in `PERF0-BASELINE.md`.
