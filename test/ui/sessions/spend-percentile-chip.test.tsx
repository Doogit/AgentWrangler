...
describe("SpendPercentileChip ranking boundaries", () => {
  ...
  it("clamps the highest-value session to 'top 1%' rather than 'top 0%'", () => {
    render(<SpendPercentileChip percentile={1} n={25} />);
    const chip = screen.getByText("top 1% by estimated value");
    expect(chip.getAttribute("title")).toContain("estimated value at or above 100%");
  });
  ...
  it("never renders 'top 100%' for a cheap session in a large workspace", () => {
    ...
    expect(screen.queryByText(/top 100% by estimated value/)).toBeNull();
    const chip = screen.getByText("top 99% by estimated value");
    expect(chip.getAttribute("title")).toContain("estimated value at or above 1%");
  });
  ...
  it("keeps the label and tooltip complementary at a .5 rounding boundary", () => {
    ...
    const chip = screen.getByText("top 7% by estimated value");
    expect(chip.getAttribute("title")).toContain("estimated value at or above 93%");
  });

  it("carries n and the window into the tooltip", () => {
    render(<SpendPercentileChip percentile={0.9} n={30} windowDays={90} />);
    const chip = screen.getByText("top 10% by estimated value");
