...
**Reconciled build plans (implementation authority):** `docs/plans/spec-recommendations-engine.md`, `docs/plans/Archive/phase-1a-implementation-plan.md`, `docs/plans/Archive/detector-engine-wp.md`, `docs/plans/Archive/wp5-t4-build-plan.md`, `docs/plans/Archive/roadmap-to-prd.md` — superseded by `docs/plans/00-MASTER-PLAN.md`  
...
> **v0.7.2 note.** This revision reconciles the PRD with the four build plans above and the 2026-08-21/25 research passes (`docs/research/research-prompt-{A,B}-*`). The load-bearing correction: **rate-limit headroom, not raw token count, is the resource to optimize** — cache reads draw against the weekly cap at the *cached* rate (~0.1×), so savings are valued cap-weighted, not at full token weight. See §9.1a, §10.1, and the §20 change log.
...
v0.6.x specified a control-and-evidence loop (sandbox, policy, model admission, managed execution, enforced verification). Operating experience reframed the highest-value first problem: the user hit a weekly usage limit three days early, and a one-off transcript analysis (`token-usage-review-2026-08-21.md`) located the causes — 98% of tokens were context cache re-reads, 85% of spend on a premium model contrary to the user's own routing rule, oversized always-loaded context, and long full-context sessions. That analysis was ad-hoc; the product opportunity is making it continuous, trustworthy, and closed-loop.
...
**G-3 — Live burn visibility.** Active sessions (typically 2–3 workspaces, multiple sessions each) are visible with running cost, context size, and a weekly-limit burn forecast — the signal whose absence caused the early limit exhaustion.  
...
- 2–3 workspaces active concurrently, multiple sessions per workspace, ~10–12k assistant turns/week across ~120 sessions;
- interactive Claude Code (terminal/IDE), long sessions, heavy context caching (≈98% of tokens are cache reads at ~240k context/turn);
- subscription (rate-limited) billing, so **tokens ≈ quota**, and week-day burst patterns matter (57% of a week's spend in one day, limit exhausted 3 days early);
- an existing personal routing rule (CLAUDE.md: cheap models for search/extraction, premium for hard reasoning) that is **not being followed** (85% premium-model spend) — adherence is therefore a measurable metric, not a hypothetical.
...
Subscription billing rate-limits on a weekly token cap, but token classes do **not** draw against that cap equally: cache reads draw at the **cached rate (~0.1×)**, while input, output, and cache **writes** draw at full weight. Because ≈98% of observed tokens are cache reads, a raw-token or full-weight "tokens saved" figure overstates recoverable headroom by roughly an order of magnitude (the 2026-08-25 economics brief; the "tokenmaxxing" trap).
...
**NFR-104** Initial back-scan of a corpus at the observed scale (26 project dirs, ~16k turns) completes in minutes, not hours.  
...
2. Live burn + forecast would have flagged the limit-exhaustion week **before** day 3.
...
5. At least 3 recommendations adopted with **measured** effects; the 30–45% weekly-reduction envelope from the review is confirmed, revised, or falsified by measured data — any of those outcomes is a valid product-learning result and is not masked.
...
8. **Success criteria anchored to the operator's real incident — ACCEPTED (§15).** Reproduce the weekly review automatically; demonstrate the forecast would have pre-empted the early limit exhaustion; confirm/revise/falsify the 30–45% envelope with measured data.
